...
CompositeTrend <- function(indata, output_path, trend_choice = "arithmetic_logit_occ", group_name,
  ...
  for (j in 1:length(unique(samp_post$iter))) {
    ...
    t_table <- subset(temp_table, select = -c(spp, iter)) # convert shape of the table
  ...
  if (any(non_numeric_check)) {
    ...
    # Subset the table to exclude certain columns
    logit_temp_table <- t_table

    # If all columns are numeric, proceed with logit transformation
