...
This document shows how to use the indicator pipeline to create biodiversiy indicators such as those for DEFRA's [Biodiversity Indicators in Your Pocket](http://jncc.defra.gov.uk/page-1824). The pipeline is shared in the form of an R package called 'BRCindicators' making it easy to share and maintain.
...
To create an indicator we first need to have species trends, let's create some using hte sparta R package.
...
We have demonstrated how you might run the indicator functions one at a time now however in a 'pipeline we want data to flow through seamlessly. Addtionally there are a number of parameters in the functions that we ahev not shown you that you might find useful. Here is an example of how you can create your own pipeling function. Our function will wrap around the functions described above, setting the parameters to meet our needs. Once we have done this it will allow use to execute our pipeline in one line.
